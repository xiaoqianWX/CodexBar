...
extension UsageMenuCardView.Model {
        ...
        let sessionLine: String = {
            if provider == .bedrock {
                ...
                if let sessionTokens {
                    return String(format: L("%1$@: %2$@ · %3$@ tokens"), label, sessionCost, sessionTokens)
                }
                return String(format: L("%1$@: %2$@"), label, sessionCost)
            }
            if let sessionTokens {
    ...
    static func tokenUsageHint(provider: UsageProvider) -> String? {
        ...
        case .vertexai:
            L(UsageFormatter.costEstimateHint)
        case .bedrock:
            L("Reported by AWS Cost Explorer; daily billing data can lag.")
        default:
            nil
        }
    }
